...
- [ ] `flow-context` exists as an installable Flow skill.
- [ ] The skill scans common local sources such as README files, docs, package/config files, routes, components, services, models, schemas, tests, and agent guidance.
- [ ] The skill creates draft context for product purpose, users, workflows, architecture, codebase patterns, UI patterns, testing patterns, and open questions.
- [ ] Important claims include evidence references.
- [ ] Important claims include confidence levels: `High`, `Medium`, `Low`, or `Unknown`.
- [ ] Low- and medium-confidence claims become verification questions.
- [ ] The skill avoids asking the user for information that can be found locally.
